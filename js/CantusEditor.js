...
window.onload = function () {
    ...
    var isProduction = document.URL === productionURL;
    if (!isProduction) {
    ...
    function switchMeiTab(ignore, fileName) {
        if (!$("#auto-load-checkbox").prop('checked')) {
            return;
        }
        ...
        if (selectedManuscript.meiFiles.indexOf(fileName.split(".")[0] + ".mei") > -1) {
            ...
            if (meiEditor.getPageTitles().indexOf(meiPage) >= 0) {
                return;
            }
    ...
    function newZone(pageRef, prevID, nextID, curID) {
        ...
        if (nextID) {
            ...
            nextIdx = Array.prototype.indexOf.call(
                nextNeume.parentElement.childNodes,
                nextNeume
            );
        }
        ...
        if (prevID) {
            ...
            prevIdx = Array.prototype.indexOf.call(
                prevNeume.parentElement.childNodes,
                prevNeume
            );
        }
        ...
        if (nextID) {
            ...
            var inserted = prevNeume.insertAdjacentElement(
                "afterEnd",
                toInsert
            );
    ...
    function meiEditorLoaded() {
        ...
        $("#server-load-dropdown").click(function () {
